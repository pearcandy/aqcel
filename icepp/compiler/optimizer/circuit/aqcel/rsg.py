...
class recognition():
    def __init__(self, circuit, n_patterns, level, min_n_repetition,
    ...
    def get_node_list(self, dag, node, max_depth, i_depth=0):
        ...
        result = set()
        result_children = [
            self.get_node_list(dag, child, max_depth - 1, i_depth + 1)
            for child in dag.successors(node)
            if (child.type != 'out' and child.name != 'measure')
        ]

        # All combinations between elements in result_children
        for bit in itertools.product([False, True],
                                     repeat=len(result_children)):
            ...
            comb_list = [[
                u for u in v if len(u) < self.max_num_nodes - i_depth
            ] for v in comb_list]

            for comb in itertools.product(
                    *comb_list):  # TODO: Bottleneck of calculation time
                r = []
                for v in comb:
                    r.extend(v)
        ...
        for node_set in list(result):
            ret = self.check_bad_nodeset(node_set, self.dag,
                                         self.max_num_nodes, max_depth)
            if ret is not None:
        ...
        return result
<<<<<<< HEAD:icepp/compiler/optimizer/circuit/aqcel/rsg.py
...
>>>>>>> a7787c799eb5cc13730903e691e258ee09342c0a:aqcel/optimization/pattern.py
    ...
    def quantum_pattern(self):
        ...
        if len(
                self.subdag_dict
        ) == 0:  # If there is no recurring gate, output meaningless circuit
    ...
    def gate_set_finder(self):
        ...
        for j in range(len(self.circuit)):
            ...
            for i, gate_list in enumerate(newgate_list):
                if (self.circuit[j][0].name == gate_list[0][2]) & (
                        self.circuit[j][1] == gate_list[0][3]):
                    gate_index_list[i] += 1
                    if gate_index_list[i] == gate_list[0][0]:
                        circ2.barrier(*self.circuit.qregs)
                        circ2.append(self.circuit[j][0], self.circuit[j][1],
                                     self.circuit[j][2])
                        gate_list[1].pop(0)
                        index_list[i] = j
                        ...
                        for l in range(gate_list[0][1] - 1):
                            for k in range(len(self.circuit) - j):
                                if delete_list.count(j + k) == 0:
                                    if (self.circuit[j + k][0].name
                                            == gate_list[1][l].name) & (
                                                self.circuit[j + k][1]
                                                == gate_list[1][l].qargs):
                                        circ2.append(self.circuit[j + k][0],
                                                     self.circuit[j + k][1],
                                                     self.circuit[j + k][2])
